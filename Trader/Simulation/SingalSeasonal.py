...
class SignalSeasonal:
    ...
    def trade_modes(self, prices):
        modes = []
        for mode in self.q_modes:

            # smooth_mode = pd.DataFrame(prices).ewm(alpha=mode).mean()
            # modes.append(smooth_mode.to_numpy())

            smooth_mode = self.numpy_ewma(prices,mode)
            modes.append(smooth_mode)

        return modes

    def numpy_ewma(self,data, alpha, offset=None, dtype=None, order='C', out=None):
        """
